...
void mtk::gpu_logger_hip::init() {
	HIPTF_CHECK_ERROR(rsmi_init(0));
}

void mtk::gpu_logger_hip::shutdown() {
}

std::size_t mtk::gpu_logger_hip::get_num_devices() const {
	return hiptf::device::get_num_devices();
}

double mtk::gpu_logger_hip::get_current_temperature(const unsigned gpu_id) const {
	int64_t temperature;
...
double mtk::gpu_logger_hip::get_current_power(const unsigned gpu_id) const {
	uint64_t power;
...
std::size_t mtk::gpu_logger_hip::get_current_used_memory(const unsigned gpu_id) const {
	uint64_t used;
